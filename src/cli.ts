...
function showBanner(): void {
    ...
    const C_BLUE = '\x1b[38;5;63m';
    const C_CYAN = '\x1b[38;5;87m';
    const C_RESET = '\x1b[0m';

    console.log('');
    console.log(`${C_PURPLE}██╗   ██╗██╗██████╗ ███████╗    ${C_BLUE}███████╗██╗      ██████╗ ██╗    ██╗${C_RESET}`);
    console.log(`${C_PURPLE}██║   ██║██║██╔══██╗██╔════╝    ${C_BLUE}██╔════╝██║     ██╔═══██╗██║    ██║${C_RESET}`);
    console.log(`${C_PURPLE}██║   ██║██║██████╔╝█████╗      ${C_BLUE}█████╗  ██║     ██║   ██║██║ █╗ ██║${C_RESET}`);
    console.log(`${C_PURPLE}╚██╗ ██╔╝██║██╔══██╗██╔══╝      ${C_BLUE}██╔══╝  ██║     ██║   ██║██║███╗██║${C_RESET}`);
    console.log(`${C_PURPLE} ╚████╔╝ ██║██████╔╝███████╗    ${C_BLUE}██║     ███████╗╚██████╔╝╚███╔███╔╝${C_RESET}`);
    console.log(`${C_PURPLE}  ╚═══╝  ╚═╝╚═════╝ ╚══════╝    ${C_BLUE}╚═╝     ╚══════╝ ╚═════╝  ╚══╝╚══╝${C_RESET}`);
    console.log(`${C_CYAN}           [ AI ARCHITECT ] • [ SLEEP MODE: ON ] • [ 🌙 zzz ]${C_RESET}`);
    console.log('');
}
...
function showHelp(): void {
    ...
    console.log('');
    log.info('Interactive Debug Mode:');
    log.success('  Press Ctrl+T during task execution to enter debug mode');
    log.success('  Then you can: view logs, debug tasks, attach to sessions');
    console.log('');
}
